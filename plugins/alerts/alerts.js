...
module.exports = {
    ...
    execute(client, data) {
        ...
        if ((data.chatType === FOLLOW ||
            ...
            if (viewer.vip) {
                try {
                    const uniques = JSON.parse(fs.readFileSync(path.join(__dirname, 'uniquejoin.json'), "utf8"));
                    for (name in uniques.unique) {
                        if (name === viewer) {
                            message = uniques.unique[x];
                            changed = true;
                        }
                    }
                } catch (e) {
                    Bot.log(`things didn't go so well in unique viewer message alerts.js`);
                }
            }
            ...
            obsToggle(scene, source, delay);
            slobsToggle(source, delay);
            if (changed) { https("joined", data.user, message); }
            else if (!changed) { https("joined", data.user, settings.alerts.joined.httpMessage);}
        }
