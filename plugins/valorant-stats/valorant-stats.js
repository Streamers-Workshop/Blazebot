      ...
      https.get(userUrl, (resp) => {
        ...
        resp.on('end', () => {
          ...
          if (obj.data) {
            ...
            client.sendMessage(`@${data.user} here are your Valorant stats:\r Current Rank: ${rank}\rK/D Ratio: ${kdr}% \r Win Ratio: ${winr}\r Total Wins: ${wins}\r Total Kills: ${kills}\r Total Deaths: ${deaths}\r Most Kills: ${mKills}`);
          } else {
            const err = obj.errors[0];
            const errMsg = err.message;
            if (errMsg) {
              client.sendMessage("We could not find the player " + `${data.args[0]}` + ". Make sure you have our Overwolf App!")
            }
          }
        });
      }).on("error", (err) => {
