...
const bJson = JSON.parse(
  fs.readFileSync('./chatmessagefilters/bannedWords.json', 'utf8'),
);
...
module.exports = {
  ...
  execute(message, bot, plugins, settings) {
    if (!settings.BADWORDS || settings.BADWORDS === 0) return false;
    ...
    if (bWords.some((word) => message.content.includes(word))) {
