...
if (process.env.OBS_ACTIVE > 0) {
  const obsSettings = require('./events/obs.Settings.json');

  obs.connect({
    address: obsSettings.address,
...
const messageFilterFiles = fs
  .readdirSync(path.join(__dirname, 'chatmessagefilters'))
  .filter((file) => file.endsWith('.js'));
messageFilterFiles.forEach((file) => {
  const filter = require(path.join(
    path.join(__dirname, 'chatmessagefilters'),
    file,
  ));
...
bot.on('chatEvent', (type, data) => {
  bot.text_events.forEach((value, key) => {
    if (!value.event) return;
    if (value.event === data.chatType) {
      const trigger = bot.text_events.get(key);
      try {
...
bot.on('chatMessage', (message) => {
  ...
  if (
    process.env.TROVO_PREFIX &&
    message.content.startsWith(process.env.TROVO_PREFIX)
  ) {
    // it's a command

    const args = message.content
      .slice(process.env.TROVO_PREFIX.length)
      .split(/ +/);
    ...
    if (
      ...
      (!message.badges ||
        command.permissions.filter((value) => message.badges.includes(value))
          .length === 0)
    ) {
    ...
    const filterIterator = bot.chat_message_filters.values();
    let result = filterIterator.next();
    while (!result.done) {
      const filter = result.value;
      try {
        if (
          !message.badges ||
          filter.excludeBadges.filter((value) => message.badges.includes(value))
            .length === 0
        ) {
          if (
            filter.execute(
        ...
        console.log('chatMessage', filter.name, e);
      }
      result = filterIterator.next();
    }
  }
});
...
bot.login(
  process.env.TROVO_PAGE,
  process.env.TROVO_EMAIL,
  process.env.TROVO_PASSWORD,
);
